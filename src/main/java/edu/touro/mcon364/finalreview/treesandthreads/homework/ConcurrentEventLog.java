 ...
 * meaning you lose the event entirely thats is why we need a composite key
 ...
 * (earlier dates = smaller numbers) and Long sorts with smallest number first so no sorting step needed
 * chronological order is built in
 * this is relevent for getEventsAfter, getEvents Between and getMost recentN
 * <p>
 * <p>
 ...
 * <p>
 * live = window into the original map and it is safe to use here bc all writers stop before the query
 * <p>
 * Requirements:
...
public class ConcurrentEventLog {
    ...
    // ConcurrentSkipListMap is the thread-safe version of TreeMap, threads can read and write safely simultaneously
    ...
    // getAndIncrement() reads and increments automatically
    ...
    public void logEvent(long timestamp, String message) {
        // needs a composite key here so that two events dont share the same Long key
        // Multiplying by 1_000_000L "spreads out" each millisecond into a million
        // slots, so the sequence number fits in between without pushing the key
        // into the next millisecond's range. Chronological order is preserved
        // because a later timestamp will always produce a larger key.
    ...
    public void runConcurrentSources(List<String> sources, int eventsEach)
        ...
        // creating one thread per CPU
        ExecutorService pool = Executors.newFixedThreadPool(Runtime.getRuntime().availableProcessors());
        sources.stream()
                .forEach(sourceName -> pool.submit(() ->
                        IntStream.range(0, eventsEach)
                                .forEach(i -> logEvent(System.currentTimeMillis(), sourceName + "-" + i))
                ));
    ...
    public List<String> getEventsAfter(long timestamp) {
        // calling tailMap on my ConcurrentSkipListMap
        // using false to exclude the given timestamp bc it otherwise included
        // make sure that we are using the composite key!! to match format
        // .values() give just the messages not the Long keys
        //List.copy() wraps it in immutable list
        return List.copyOf(log.tailMap(timestamp * 1_000_000L, false).values());
    }
    ...
    public List<String> getEventsBetween(long from, long to) {
        // careful here bc ew have a composite key
        // so we scale the bounds to match composite key format
        return List.copyOf(log.subMap(from * 1_000_000L, true, to * 1_000_000L + 999_999L, true).values());
    }
    ...
    public List<String> getMostRecentN(int n) {
        // log iterates oldest first!! (smallest Log key first)
        return log.descendingMap() // slips so that we get newest first (largest key first)
                .values() // just messages
                .stream()
                .limit(n)
                .toList();
    }
